...
URL = "opc.udp://10.188.154.140:4840"
...
async def main():
    ...
    server = Server()
    await server.init()
    server.set_endpoint("opc.tcp://10.188.154.140:4840/freeopcua/server/")
    ...
    async with server:
        ...
        con = pubsub.PubSubConnection.udp_uadp(
            ...
            writer_groups=[
                ...
                pubsub.WriterGroup.new_uadp(
                    name="WriterGroup1",
                    writer_group_id=ua.UInt16(1),
                    publishing_interval=5000,
                    writer=[
                        pubsub.DataSetWriter.new_uadp(
                            name="Writer1",
                            dataset_writer_id=ua.UInt16(32),
        ...
        while 1:
            ...
            count += 1
            await nodes[0].write_value(count, ua.VariantType.Int32)
            await nodes[1].write_value(f"DemoString {count}", ua.VariantType.String)
            await nodes[2].write_value(count % 2 == 0, ua.VariantType.Boolean)
            await nodes[3].write_value([count, count + 1, count + 2], ua.VariantType.Int16)
            print(f"Publisher updated nodes: Int32={count}, String='DemoString {count}', Bool={count % 2 == 0}, ArrayInt16={[count, count + 1, count + 2]}")
